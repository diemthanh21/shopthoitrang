...
router.post('/return-media', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    ...
    const bucket = process.env.RETURN_MEDIA_BUCKET || 'minchunghinhanh';
    const contentType = req.file.mimetype || 'application/octet-stream';

    const data = await storageService.uploadReturnMedia(bucket, objectPath, req.file.buffer, contentType);

    // respond with a relative path which mobile already expects (e.g. "minchunghinhanh/<orderId>/...")
    // supabase.storage.upload returns data.path on success; if not present, return objectPath
    const path = (data && data.path) ? data.path : objectPath;
