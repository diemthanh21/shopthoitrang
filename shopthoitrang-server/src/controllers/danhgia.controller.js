const service = require('../services/danhgia.service');
...
const DanhGiaController = {
  ...
  async findBySanPham(req, res) {
    const list = await service.findBySanPham(req.params.maSP);
    res.json(list.map(i => i.toJSON()));
  },
  ...
  async findByChiTietDonHang(req, res) {
    const list = await service.findByChiTietDonHang(req.params.maCTDH);
    res.json(list.map(i => i.toJSON()));
  },
