const service = require('../services/tukhoatimkiem.service');
...
exports.getById = async (req, res) => {
  const result = await service.layTheoMa(req.params.ma);
...
exports.update = async (req, res) => {
  const result = await service.capNhat(req.params.ma, req.body);
  if (!result) return res.status(400).json({ message: 'Cập nhật thất bại' });
  res.json(result);
};

exports.delete = async (req, res) => {
  const result = await service.xoa(req.params.ma);
